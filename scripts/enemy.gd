...
func _ready() -> void:
	...
	rotate_speed = randf_range(2, 5)
	rotate_dir = [-1, 1].pick_random()
	...
	if body.is_in_group("player"):
		...
		if body.get_rotate_dir() == rotate_dir: # if same direction
			if player_rotate_speed > rotate_speed: # if player faster than enemy
				body.add_rotate_speed(-rotate_speed / 8) # slow down player
				...
				add_rotate_speed(-player_rotate_speed / 8) # slow down enemy
			...
			body.add_rotate_speed(rotate_speed / 2) # add speed if player is going opposite
			add_rotate_speed(-player_rotate_speed / 4) # slow down enemy 
